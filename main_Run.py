...
# For taking images for datasets
def take_img():
    l1 = txt.get()
    l2 = txt2.get()
    if l1 == '' or l2 == '':
        err_screen()  # Assuming err_screen is defined elsewhere
        return
    ...
    try:
        ...
        while True:
            ...
            if sampleNum > 70:  # Limit to 70 samples
                break
...
def training():
    ...
    # Load images and corresponding labels for training
    recognizer = cv2.face.LBPHFaceRecognizer_create()  # Ensure you have opencv-contrib-python installed
    ...
    # Load student details from CSV file for attendance tracking
    df = pd.read_csv("StudentDetails/StudentDetails.csv")
    
    # Assuming you have logic here to populate `faces` and `Ids`
...
# Ensure that the paths used in the code are correct and exist.
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='grey80')
                cs = r"C:\Users\HP\Desktop\face recognition\Attendance" + fileName
    ...
    def log_in():
        ...
        if username == 'arpit':
            if password == 'arpit123':
                ...
                cs = r"C:\Users\HP\Desktop\face recognition\StudentDetails\StudentDetails.csv"
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
